...
f = 2*pi*(0:L/2)/L; f = f(2:end);                                                       % Frequency spectrum (normalized over coiling frequency)
P1X0_n = P1X0/max([P1X0;P1Y0]);                                                         % Normalization of amplitudes over max value
P1Y0_n = P1Y0/max([P1X0;P1Y0]);

figure(3); plot(f,P1X0_n); hold on; plot(f,P1Y0_n); legend('wx','wy'); hold off         % FFT plot - NO FORCING
...
while P1Y0_n(j) > rms(P1Y0_n)                                                           % This frequency we will call the pattern frequency.
    j = j + 1; end
[A,B] = max(P1Y0_n(i-1:j)); FR0 = f(i+B-2); fp = fc*FR0;                                % FR0 is defined as the pattern frequency ratio (pattern frequency/coiling frequency)
...
Y1 = Y(:,1).*sin(Y(:,2)) + Yc; Y2 = Y1;                                                 % X-coordinate contact point trace / deposited trace
...
figure(6); plot(f7FR0,P1X_n); hold on; plot(f/FR0,P1Y_n); legend('wx','wy'); hold off   % FFT plot - FORCING
                                                                                        % Frequency axis is normalized over pattern frequency.
